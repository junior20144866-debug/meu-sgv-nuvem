...
st.set_page_config(page_title="JMQJ SGV v63", layout="wide", page_icon="📈")

# --- 2. MOTOR DE RECUPERAÇÃO (BUSCA SEGURA) ---
def carregar_dados_v63():
    try:
        # Busca Config: Sem filtrar por ID no select para evitar erro se a tabela estiver vazia
...
if not st.session_state.auth:
    ...
    emp, df_e, df_c = carregar_dados_v63()
    ...
    # --- ABA: VENDAS (MODELO BÚSSOLA REFINADO) ---
    if menu == "💰 Vendas & Pedidos":
        ...
        if btn_pdf:
            ...
            with col_v2:
                ...
                <div class="paper">
                    <table style="width:100%">
                        <tr>
                            ...
                            <td style="text-align:center">
                                <b style="font-size:16px">{emp.get('nome', 'EMPRESA')}</b><br>
                                CNPJ: {emp.get('cnpj', '')}<br>{emp.get('end', '')}
                            </td>
                            <td style="text-align:right; border:1px solid #000; padding:5px">
                                PEDIDO: 004215<br>Data: {datetime.now().strftime('%d/%m/%Y')}<br>Venc: {venc}<br><b>{vias}</b>
                            </td>
                        </tr>
                    </table>
                    <hr>
                    ...
                    <br><div style="text-align:right"><b>VALOR TOTAL: R$ {float(prod_v.split('R$')[1].replace(',','.'))*qtd:,.2f}</b></div>
                </div>
                """, unsafe_allow_html=True)

    # --- ABA: CLIENTES (FORÇANDO O DESPERTAR) ---
    elif menu == "👥 Clientes":
    ...
    # --- ABA: AJUSTES (RESOLVENDO O ERRO 23502) ---
        ...
        with st.form("f_emp"):
            ...
            if st.form_submit_button("💾 FIXAR DADOS"):
                ...
                # SOLUÇÃO DEFINITIVA: Forçamos o ID 1 no UPSERT
                try:
    ...
    # --- OUTROS MENUS ---
        ...
        c1, c2 = st.columns(2); c1.metric("Produtos", len(df_e)); c2.metric("Clientes", len(df_c))
    elif menu == "📦 Estoque":
        st.header("📦 Estoque"); st.dataframe(df_e, use_container_width=True, hide_index=True)
