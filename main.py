...
st.set_page_config(page_title="JMQJ SGV v57", layout="wide", page_icon="📈")

# --- 2. MOTOR DE RECUPERAÇÃO (PREVENÇÃO DE API ERROR) ---
def buscar_dados_v57():
    try:
        # Configuração: Tenta buscar, se falhar ou estiver vazio, ignora o erro para não travar
        c = supabase.table("config").select("*").execute().data
        empresa = c[0] if c else {"id": 1, "nome": "JMQJ SGV", "cnpj": "", "end": "", "wts": ""}
        
        # Produtos e Clientes: Busca bruta
...
if not st.session_state.auth:
    ...
    emp, df_e, df_c = buscar_dados_v57()
    ...
    # --- VENDAS & FINANCEIRO (RECONSTRUÍDO) ---
    elif menu == "💰 Vendas & Financeiro":
        st.header("💰 Módulo Financeiro e Vendas")
        col_f1, col_f2 = st.columns([2, 1])
        with col_f1:
            st.subheader("Novo Lançamento")
            # Busca dinâmica de clientes para o financeiro
            cli_f = st.selectbox("Cliente", df_c['nome_completo'].tolist() if not df_c.empty else ["Consumidor"])
            prod_f = st.selectbox("Produto", [f"{p['descricao']} | R$ {p['preco_venda']}" for p in df_e.to_dict('records')] if not df_e.empty else ["Sem Produtos"])
        ...
        with col_f2:
            ...
            vias = st.radio("Gerar PDF", ["1 Via", "2 Vias"], horizontal=True)
            if st.button("✅ CONCLUIR VENDA"): st.success("Registrado no Financeiro!")
    ...
    # --- CLIENTES (FIXAÇÃO DE INCLUSÃO MANUAL) ---
    elif menu == "👥 Clientes":
    ...
    # --- AJUSTES & RESET (FIXAÇÃO DA EMPRESA) ---
        ...
        with st.form("f_emp"):
            n_e = st.text_input("Nome Empresa", value=emp.get('nome', ''))
            ...
            if st.form_submit_button("💾 FIXAR DADOS"):
                ...
                # O Segredo: Remover o ID do payload se o banco for autoincremento ou usar upsert puro
