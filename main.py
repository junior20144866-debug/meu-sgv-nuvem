...
st.set_page_config(page_title="JMQJ SGV v55", layout="wide", page_icon="📈")

# --- 2. MOTOR DE RECUPERAÇÃO E FIXAÇÃO (ANTI-GARGALO) ---
def buscar_dados_v55():
    try:
        # Força a criação/busca da configuração id=1
        c = supabase.table("config").select("*").eq("id", 1).execute().data
        if not c:
            # Se não existir, tenta criar o registro mestre para evitar erro de fixação
            supabase.table("config").insert({"id": 1, "nome": "JMQJ SGV"}).execute()
            c = supabase.table("config").select("*").eq("id", 1).execute().data
...
if not st.session_state.auth:
    ...
    emp, df_e, df_c = buscar_dados_v55()
    ...
    # --- VENDAS & FINANCEIRO (COM PDF) ---
        ...
        with col1:
            ...
            p_v = st.selectbox("Produto", [f"{p['descricao']} | R$ {p['preco_venda']}" for p in df_e.to_dict('records')] if not df_e.empty else ["Vazio"])
            qtd = st.number_input("Quantidade", min_value=1, value=1)
            f_p = st.selectbox("Forma de Pagamento", ["Dinheiro", "Pix", "Cartão", "A Prazo"])
        with col2:
            ...
            if st.button("✅ FINALIZAR E GERAR PDF"):
                st.info("Simulando geração de PDF para impressão...")
        ...
        with tab_e:
            ...
            with st.form("f_p"):
                ...
                if st.form_submit_button("💾 SALVAR PRODUTO"):
                    supabase.table("produtos").upsert({"id": it['id'] if it else None, "descricao": d, "unidade": u, "preco_venda": p}).execute()
                    st.rerun()

    # --- CLIENTES (FIXAÇÃO DE INCLUSÃO E IMPORTAÇÃO) ---
    elif menu == "👥 Clientes":
        ...
        with tab_ce:
            ...
            with st.form("f_c"):
                # Usei nomes genéricos e Fallback para garantir a gravação
            ...
            for r in df_in.to_dict('records'):
                try:
                    if alvo == "produtos":
                        ...
                        # Mapeamento DNA Siscom -> JMQJ SGV
    ...
    # --- AJUSTES & RESET (FIXAÇÃO DA EMPRESA) ---
        ...
        with st.form("f_emp"):
            ...
            if st.form_submit_button("💾 FIXAR DADOS DA EMPRESA"):
                ...
                # O segredo da fixação: Upsert garantido no ID 1
